...
const GraphForReviews = () => {
    ...
    const exclude = (smallArr) => {
        ...
        if (
            (parseInt(smallArr[0]) < minArr[0] || (parseInt(smallArr[0]) === minArr[0] && parseInt(smallArr[1]) < minArr[1])) ||
            (parseInt(smallArr[0]) > maxArr[0] || (parseInt(smallArr[0]) === maxArr[0] && parseInt(smallArr[1]) > maxArr[1])) ||
            (parseInt(smallArr[0]) === maxArr[0] && parseInt(smallArr[1]) === maxArr[1] && parseInt(smallArr[2]) > maxArr[2])
        ) {
            return true;
        }
        return false;
        ...
        return (
            ...
            <div id='graphForReviews'>
                <h2 className="user_reviews_header no_bottom_margin">Customer reviews</h2> 
                    <AllReviews setMinDate={setMinDate} setMaxDate={setMaxDate} permData={permData} setPermData={setPermData} expanded={expanded}  data={reviewData} />
                    <RecentReviews setMinDate={setMinDate} setMaxDate={setMaxDate} recentPermData={recentPermData} setRecentPermData={setRecentPermData} expanded={expanded} data={reviewData} />
                    <ReviewFilters fullData={fullData} filters={filters} setFilters={setFilters} expanded={expanded} toggleExpanded={toggleExpanded} />               
            </div>
        </>
    )
    } else {
